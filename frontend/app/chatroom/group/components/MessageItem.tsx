...
interface Props {
  ...
  quotedMessage?: { sender: string; content: string };
  isSender: boolean;
  readers: string[];
  ...
  setReplyTo: React.Dispatch<React.SetStateAction<{ id: number; content: string; sender: string } | null>>;
      ...
      <div className={`flex items-end ${isSender ? "flex-row" : "flex-row-reverse"} group relative`}>
        <div className="relative z-10">
          ...
          {actionBoxVisible === msg.id && (
            ...
            >
              <button onClick={() => handleHide(msg.id)} className="hover:underline mr-2">削除</button>
              {isSender && (
                <button onClick={() => handleRevoke(msg.id)} className="hover:underline">送信取消</button>
              )}
              <button
                onClick={() => {
        ...
        <div className={`ml-2 mr-2 p-2 rounded-lg max-w-xs ${isSender ? "bg-blue-500" : "bg-green-700"} text-white`}>
          ...
          {quotedMessage && (
            <div className="mb-2 px-2 py-1 bg-white/30 rounded text-xs text-white border border-white/40">
              <span className="font-bold">@{quotedMessage.sender}</span>：{quotedMessage.content}
            </div>
          )}
